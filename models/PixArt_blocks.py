...
class MultiHeadCrossAttention(nn.Module):
    ...
    def forward(self, x, cond, mask=None):
        ...
        attn_mask = None
        if mask is not None:
            attn_mask = mask.view(B, 1, 1, -1).repeat(1, self.num_heads, N, 1)
...
class WindowAttention(Attention_):
    ...
    def forward(self, x, mask=None):
        ...
        if mask is not None:
            print("Mask need to check!")
        x = F.scaled_dot_product_attention(q, k, v, dropout_p=self.attn_drop.p, attn_mask=mask)
